  ...
  try {
    ...
    if (!workspaceFolders || workspaceFolders.length === 0) {
      vscode.window.showWarningMessage(
        "No workspace folder found, trying to get global user",
      );
    } else {
...
export function activate(context: vscode.ExtensionContext) {
  ...
  if (users.length === 0) {
    vscode.commands.executeCommand("git-profile.setupUser");
  }

  getCurrentUser()
    .then((user) => {
      statusBarItem.text = user.name || "Git Profile - No User Set";
      statusBarItem.tooltip = "Click to setup git user";
      statusBarItem.command = "git-profile.setupUser";
      statusBarItem.show();
    })
    .catch(() => {
      statusBarItem.text = "Git Profile - Setup Required";
      statusBarItem.tooltip = "Click to setup git user";
      statusBarItem.command = "git-profile.setupUser";
      statusBarItem.show();
    });
